    ...
    <Panel className="overflow-hidden bg-surface-950/60 shadow-xl shadow-black/15">
      ...
      <div className="space-y-3 p-4">
        ...
        <div className="grid grid-cols-2 gap-2.5">
          ...
          <MiniMetric label="Trade Quality" value={`${tradeQuality.score}`} />
        </div>
        ...
        <PlanMoreContext footprint={footprint} multiTimeframe={multiTimeframe} />
        <button
...
function PlanMoreContext({
  ...
  return (
    <div className="rounded-lg border border-white/10 bg-white/[0.03]">
      <button
        className="flex w-full items-center justify-between gap-2 px-3 py-2 text-left text-[11px] font-semibold text-slate-400 transition hover:text-white"
      ...
      {open ? (
        <div className="space-y-2 border-t border-white/10 px-3 py-2 text-xs leading-5 text-slate-400">
          <p>
            <span className="font-semibold text-slate-300">MTF: </span>
            {multiTimeframe.status} — see under-chart evidence card for the default takeaway.
          </p>
          <p>
            <span className="font-semibold text-slate-300">Footprint: </span>
            {footprint.type}
          </p>
        </div>
      ) : null}
